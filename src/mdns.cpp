...
/*
 * mDNS C callback handlers
 */

static int query_callback(int sock, const struct sockaddr *from, size_t addrlen, mdns_entry_type_t entry,
    uint16_t query_id, uint16_t rtype, uint16_t rclass, uint32_t ttl, const void *data, size_t size, size_t name_offset,
    size_t name_length, size_t record_offset, size_t record_length, void *user_data)
{
    ...
    MDNS_LOG << "query_callback for " << entrystr << " :: " << name_offset << ":" << record_offset;
    ...
    if (rtype == MDNS_RECORDTYPE_PTR) {
        ...
        MDNS_LOG << fromaddrstr << " : "
                 << entrytype << " "
                 << entrystr << " "
                 << printableRecordType(rtype) << " "
                 << namestr << " "
                 << "rclass 0x" << std::hex << rclass << std::dec << " "
                 << "ttl " << ttl << " "
        ...
        MDNS_LOG << "[UNHANDLED] " << fromaddrstr << " : "
                 << entrytype << " "
                 << entrystr << " "
                 << printableRecordType(rtype) << " "
                 << "rclass 0x" << std::hex << rclass << std::dec << " "
    ...
    size_t name_length, size_t record_offset, size_t record_length, void *user_data)
{
    ...
    if ((name.length == (sizeof(dns_sd) - 1)) && (strncmp(name.str, dns_sd, sizeof(dns_sd) - 1) == 0)) {
        if (enType == mdns_record_type::MDNS_RECORDTYPE_PTR || enType == mdns_record_type::MDNS_RECORDTYPE_ANY) {
            MDNS_LOG << fromaddrstr << " : question PTR SD " << std::string(name.str, name.length);
        ...
        mdns_record_t answer = { .name = {}, .type = MDNS_RECORDTYPE_IGNORE };
        std::vector<mdns_record_t> additional;

        if ((name.length == service_record->service.length) && strncmp(name.str, service_record->service.str, name.length) == 0) {
            if (enType == mdns_record_type::MDNS_RECORDTYPE_PTR || enType == mdns_record_type::MDNS_RECORDTYPE_ANY) {
                MDNS_LOG << fromaddrstr << " : question PTR " << std::string(name.str, name.length);
                ...
                // Answer PTR record reverse mapping "<_service-name>._tcp.local." to
                // "<hostname>.<_service-name>._tcp.local."
                answer = service_record->record_ptr;

                auto srv = inst->currentSRV();
                MDNS_LOG << "add SRV " << srv.name << " : " << srv.data.srv;
                additional.emplace_back(srv);
        ...
        } else if (name.length == service_record->service_instance.length() && strncmp(name.str, service_record->service_instance.data(), name.length) == 0) {
        ...
        } else if (name.length == service_record->hostname_qualified.length() && strncmp(name.str, service_record->hostname_qualified.data(), name.length) == 0) {
            if ((enType == mdns_record_type::MDNS_RECORDTYPE_A || enType == mdns_record_type::MDNS_RECORDTYPE_ANY)
                && service_record->address_ipv4.sin_family == AF_INET) {
                MDNS_LOG << fromaddrstr << " : question A " << std::string(service_record->hostname_qualified);
                // The A query was for our qualified hostname (typically
                // "<hostname>.local.") and we have an IPv4 address, answer with an A
                // record mapping the hostname to an IPv4 address, as well as any IPv6
                // address for the hostname, and two test TXT records

                // Answer A records mapping "<hostname>.local." to IPv4 address
                answer = service_record->record_a;
            ...
            } else if ((enType == mdns_record_type::MDNS_RECORDTYPE_AAAA || enType == mdns_record_type::MDNS_RECORDTYPE_ANY)
                && service_record->address_ipv6.sin6_family == AF_INET6) {
                MDNS_LOG << fromaddrstr << " : question AAAA " << std::string(service_record->hostname_qualified);

                // The AAAA query was for our qualified hostname (typically
                // "<hostname>.local.") and we have an IPv6 address, answer with an AAAA
                // record mapping the hostname to an IPv6 address, as well as any IPv4
                // address for the hostname, and two test TXT records

                // Answer AAAA records mapping "<hostname>.local." to IPv6 address
                answer = service_record->record_aaaa;
        ...
        if (answer.type != MDNS_RECORDTYPE_IGNORE) {
            // Always include TXT records in additional
            ...
            uint16_t unicast = (rclass & MDNS_UNICAST_RESPONSE);
            MDNS_LOG << "  --> answer "
                     << service_record->hostname << "." << service_record->service
                     << " port " << service_record->port
                     << " (" << (unicast ? "unicast" : "multicast") << ")";
            if (unicast) {
                mdns_query_answer_unicast(sock, from, addrlen, sendbuffer, sizeof(sendbuffer), query_id, enType,
                    name.str, name.length, answer, 0, 0, &additional[0], additional.size());
            } else {
...
void mDNS::startService()
{
    ...
    running_ = true;
    worker_thread_ = std::thread([this] { this->runMainServiceLoop(); });
}
...
bool mDNS::isServiceRunning() const { return running_; }
...
void mDNS::setSRVPriorityCallback(std::function<std::uint16_t()> cb) { getSRVPriority_ = cb; }
void mDNS::setSRVWeightCallback(std::function<std::uint16_t()> cb) { getSRVWeight_ = cb; }
const ServiceRecord *mDNS::serviceRecord() const { return service_record_.get(); }

mdns_record_t mDNS::currentSRV()
{
    auto base = service_record_->record_srv;

    MDNS_LOG << "base SRV " << base.name << " : " << base.data.srv;
        ...
        if (res > 0) {
            for (int isock = 0; isock < num_sockets; ++isock) {
                if (FD_ISSET(sockets[isock], &readfs)) {
                    ...
                    records += mdns_query_recv(sockets[isock], buffer, capacity, query_callback, &query_result, query_id[isock]);
    ...
    do {
        ...
        if (res > 0) {
            for (int isock = 0; isock < num_sockets; ++isock) {
                if (FD_ISSET(sockets[isock], &readfs)) {
                    QueryResult query_result;
                    records += mdns_discovery_recv(sockets[isock], buffer, capacity, query_callback, &query_result);
                    handle_result(query_result);
                }
            }
        }
...
void mDNS::runMainServiceLoop()
{
    ...
    MDNS_LOG << "Opened " << std::to_string(num_sockets) << " socket" << (num_sockets ? "s" : "") << " for mDNS service";
...
void mDNS::initServiceRecord()
{
    ...
    MDNS_LOG << "initServiceRecord [" << service_record_->service << "][" << service_record_->hostname << "]";
    ...
    service_record_->service_instance = std::string(service_instance_buffer);

    MDNS_LOG << "  service_instance [" << service_record_->service_instance << "]";

    // Build the "<hostname>.local." string
    char qualified_hostname_buffer[256] = { 0 };
    snprintf(qualified_hostname_buffer, sizeof(qualified_hostname_buffer) - 1, "%.*s.local.", MDNS_STRING_FORMAT(service_record_->hostname));
    service_record_->hostname_qualified = std::string(qualified_hostname_buffer);

    MDNS_LOG << "  hostname_qualified [" << service_record_->hostname_qualified << "]";
    ...
    // Setup our DNS records

    // PTR record reverse mapping "<_service-name>._tcp.local." to
    // "<hostname>.<_service-name>._tcp.local."
    service_record_->record_ptr = {
        .name = service_record_->service,
        .type = MDNS_RECORDTYPE_PTR,
        .data = { .ptr = { .name = { .str = service_record_->service_instance.data(), .length = service_record_->service_instance.length() } } }
    };

    // SRV record mapping "<hostname>.<_service-name>._tcp.local." to
    // "<hostname>.local." with port. Set weight & priority to 0.
    service_record_->record_srv = {
        .name = { .str = service_record_->service_instance.data(), .length = service_record_->service_instance.length() },
        ...
        .data = {
            .srv = {
                ...
                .name = { .str = service_record_->hostname_qualified.data(), .length = service_record_->hostname_qualified.length() } } }
    };
    MDNS_LOG << "  service_instance [" << service_record_->service_instance << "]";
