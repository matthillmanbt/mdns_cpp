    ...
    size_t name_length, size_t record_offset, size_t record_length, void* user_data)
{
    ...
    if (rtype == MDNS_RECORDTYPE_PTR) {
        ...
        MDNS_LOG << fromaddrstr << " : "
                 ...
                 << namestr.str << " "
                 << "rclass 0x" << std::hex << rtype << std::dec << " "
                 << "ttl " << ttl << " "
        ...
        for (size_t itxt = 0; itxt < parsed; ++itxt) {
            ...
            if (txtbuffer[itxt].value.length) {
                MDNS_LOG << fromaddrstr << " : "
                         ...
                         << entrystr.str << " "
                         << printableRecordType(rtype) << " "
                         << txtbuffer[itxt].key << " = " << txtbuffer[itxt].value;
            } else {
                MDNS_LOG << fromaddrstr << " : "
                         << entrytype << " "
                         << entrystr.str << " "
...
std::list<std::shared_ptr<QueryResult>> mDNS::executeQuery(
            ...
            for (int isock = 0; isock < num_sockets; ++isock) {
                if (FD_ISSET(sockets[isock], &readfs)) {
                    ...
                    records += mdns_query_recv(
                        sockets[isock], buffer, capacity, query_callback, query_result.get(), query_id[isock]);
